...
class DDPG:
    ...
    def update_network_parameters(self, tau=None):
        ...
        # 目标评委网络的参数更新
        for critic_params, target_critic_params in zip(
    ...
    def choose_action(self, observation, train=True):
        ...
        state = T.as_tensor(T.from_numpy(observation)).to(device)
        action = self.actor.forward(state).squeeze()

        if train:
            noise = T.tensor(
                np.random.normal(loc=0.0, scale=self.action_noise), dtype=T.float32
            ).to(device)
    ...
    def learn(self):
        ...
        # 回放经验
        states, actions, reward, states_, terminals = (
